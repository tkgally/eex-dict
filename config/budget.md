...
- **Cap:** US$15.00 per UTC day for paid model calls, all runs combined, measured by the billed `usage.cost` that OpenRouter returns when a request includes `usage: {"include": true}`.
