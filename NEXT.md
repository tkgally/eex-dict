...
*Rewritten 2026-09-26 by the seventy-third scheduled run (build).*

## State

- 493 entries, all `reviewed` (0 `draft`). Queue: 4,360 pending, 0 claimed, 493 done, 21 declined, 6 duplicate.
- This run: build, ten band-1 verbs *hear* to *hurt*; 42 adjudication decisions; 4 back-links, all correct.
...
- Spend: US$0.41 this run and today, of the US$5 daily cap.
...
4. **closure**: about 917 closure-gap lemmas.
5. **lint**: next due four runs after this one (`next_mode.py` decides). Originality is due soon. Every lint: check each BACKLINK line `crossref --all --apply` prints; about one in four is wrong. `reviewer-a` `pronunciation` precision is 0.33 over 96 decisions: re-measure; under 0.30 it is switched off.
...
- This run's report: `journal/2026-09-26.md`.
