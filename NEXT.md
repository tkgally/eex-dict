...
*Rewritten 2026-09-26 by the eighty-first scheduled run (build).*

## State

- 545 entries, all `reviewed` (0 `draft`). Queue: 4,400 pending, 0 claimed, 545 done, 21 declined, 9 duplicate.
- This session: builds *invest*–*kiss* (PR #76), *know*–*light* (PR #77), *like*–*manage* ([journal](journal/2026-09-26-9.md)).
...
- Spend: US$0.39 this run; US$3.11 today, of the US$15 daily cap.

## Queue (work top-down, one unit at a time)

1. **build**: band 1 in queue order, continuing the verbs after *manage* (`queue.py next` decides). Eight to twelve entries per run. Check each definition covers every example and collocation in its sense; reviewer-a caught five that did not this run.
...
4. **lint**: next due in two runs (`next_mode.py` decides). Every lint: check each BACKLINK line `crossref --all --apply` prints; about one in four is wrong. `reviewer-a` `pronunciation` precision is 0.33 over 96 decisions: re-measure; under 0.30 it is switched off.
...
- This run's report: `journal/2026-09-26-9.md`.
