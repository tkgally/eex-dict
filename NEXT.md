...
*Rewritten 2026-09-26 by the eighty-ninth scheduled run (build).*

## State

- 583 entries, all `reviewed` (0 `draft`). Queue: 4,419 pending, 0 claimed, 583 done, 21 declined, 9 duplicate.
- This session: builds *invest*–*kiss* (PR #76), *know*–*light* (PR #77), *like*–*manage* (PR #78), review of 20 prepositions and pronouns (PR #79), build *marry*–*name* (PR #80), lint (PR #81), build *need*–*own* (PR #82), originality check (PR #83), build *pack*–*plan* (PR #84), review of 20 prepositions and pronouns (PR #85), build *play*–*produce* ([journal](journal/2026-09-26-17.md)).
...
- Spend: US$0.30 this run; US$5.81 today, of the US$15 daily cap.
...
4. **lint**: next due in two runs (`next_mode.py` decides). Every lint: check each BACKLINK line `crossref --all --apply` prints; about one in five is wrong. A review that removes a cross-reference removes its mirror too, or lint re-creates it. `reviewer-a` `pronunciation` precision is 0.34 over 103 decisions: re-measure; under 0.30 it is switched off.
...
- This run's report: `journal/2026-09-26-17.md`.
