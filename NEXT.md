...
*Rewritten 2026-09-26 by the eighty-second scheduled run (review).*

## State

- 545 entries, all `reviewed` (0 `draft`). Queue: 4,400 pending, 0 claimed, 545 done, 21 declined, 9 duplicate.
- This session: builds *invest*–*kiss* (PR #76), *know*–*light* (PR #77), *like*–*manage* (PR #78); review of 20 prepositions and pronouns ([journal](journal/2026-09-26-10.md)).
...
- Spend: US$0.55 this run; US$3.66 today, of the US$15 daily cap.
...
4. **lint**: next due in one run (`next_mode.py` decides). Every lint: check each BACKLINK line `crossref --all --apply` prints; about one in four is wrong. `reviewer-a` `pronunciation` precision is 0.33 over 96 decisions: re-measure; under 0.30 it is switched off.
...
- This run's report: `journal/2026-09-26-10.md`.
