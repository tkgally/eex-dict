...
*This is the report for the founding session of the TKG English Learner's Dictionary. It will be completed when the session's last stage (the reviews of the seed entries and the dry run of the scheduled Routine) finishes after the UTC day rolls over; if you are reading it before then, `NEXT.md` says exactly where things stand.*
...
Nothing until the session's final report, which will list the four items: enabling GitHub Pages, reviewing the seed set, scheduling the Routine, and deciding on the drafting model.
