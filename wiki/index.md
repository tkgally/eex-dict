...
- [pronounce-check-stale-flag](notes/pronounce-check-stale-flag.md) — `pronounce_check.py` adds `pronunciation-disputed` to `provenance.flags` on a disputed verdict but never removes it once a later run re-verifies the transcription; caught 2026-09-20 on `nobody-pron`. Fix due a later run.
- [review-panel-decide-substring-collision](notes/review-panel-decide-substring-collision.md) — `--decide --quote` cannot disambiguate two issues on the same field when one's quote is a substring of the other's; caught 2026-09-20 adjudicating `pick-up-phrv`. Worked around with a manual decisions.jsonl line; fix (an `--index` option) due a later run.
...
- [metrics-duplicate-calls](notes/metrics-duplicate-calls.md) — `metrics.py` has been called twice in five past `build` runs (once per `routine-prompt.md`, once from `CLAUDE.md`'s generic checklist), leaving near-empty duplicate rows in `metrics/history.jsonl` that inflate `next_mode.py`'s run counts and can trigger `lint`/`originality` one run early. Caught 2026-09-21. Fix due a later run.
- [queue-stale-claimed-rows](notes/queue-stale-claimed-rows.md) — eleven band-1 conjunctions sit at queue status `claimed` with no backing claim file anywhere, so `queue.py next` permanently skips them; neither `claim.py --prune` nor `absorb_branch.py --residue` resets a stale status. Caught 2026-09-22. Fix due a later run.
