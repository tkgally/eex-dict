...
- [reviewer-noise](notes/reviewer-noise.md) — reviewer-a called all 10 sampled definitions "copied" in the first originality check, unsupported by search evidence in 7 of 10; updates through the seventh check (2026-09-26), where asking for a quoted source ended the false calls and now yields an occasional verified "copied" call; the quote requirement joined the checklist question 2026-09-24; reviewer-a's determiner/pronoun confusion on `which-det` (2026-09-25); reviewer-b's repeated flag against the quoted headword in adaptation notes (2026-09-25).
...
- [crossref-cross-type-backlinks](notes/crossref-cross-type-backlinks.md) — `crossref.py` added a back-link of one type (synonym, compare) where the target already named the source under another, making the pair both; caught 2026-09-26 on *contain/include* and *tell/inform*, reverted by hand. Fixed 2026-09-26: the tool now reports a `TYPE-MISMATCH` line and writes nothing.
- [restart-branch-force-push](notes/restart-branch-force-push.md) — the Routine's per-cycle branch restart needs a `--force-with-lease` push, which the harness refused on 2026-09-26; worked around by merging `origin/main` into the branch after each squash merge and pushing normally.
- [homograph-queue-and-inflection](notes/homograph-queue-and-inflection.md) — `queue.tsv` has no homograph column, so `lie-v-2` (*tell an untruth*) could not be queued, and `inflect.py`'s headword-keyed exceptions would give it **lay**/**lain**; caught 2026-09-26. Fix due a later run.
