...
## 2026-09-25: twelfth lint pass — five of 19 misplaced, not yet moved

`crossref --all --apply` added 19 sense back-links (14 entries); five sit on the wrong sense: `happen-v` sense 1 <- `become-v` (*what became of him*; belongs on sense 2, *happen to someone*), `fit-v` sense 1 <- `belong-v` (fit with surroundings; sense 4, *be right or suitable*), `fix-v` sense 1 <- `cure-v` (end a problem; sense 2), `fire-v` sense 1 <- `employ-v` (antonym; sense 2, *make someone leave their job*), `give-v` sense 1 <- `lend-v` (give help or support; sense 4). In four of the five, the sense sharing the most words was sense 1 anyway or no sense scored 2. This run's scripted move of the five was refused by the session's permission check, so they stay for the next run that opens these entries (moved by hand, one entry at a time). Placement rate this pass: 74 percent, in line with the measured 77.

**Thirteenth pass (2026-09-26).** 14 back-links; 4 misplaced and moved by hand (`include-v`/`count-v` 1 -> 2, `increase-v`/`cut-v` 1 -> 2, `imagine-v`/`dream-v` 2 -> 1, and `take-v`'s second compare added to `hold-v` sense 3). The five queued from the twelfth pass were moved too. None open.

**Fourteenth pass (2026-09-26).** 26 sense back-links, all to this session's 50 new verbs and the reviewed prepositions; 5 misplaced and moved by hand (`manage-v`/`control-v` 4 -> 1, `lose-v`/`get-v` and `lose-v`/`make-v` 1 -> 2, `keep-v`/`hold-v` 1 -> 2, `meet-v`/`see-v` 2 -> 1). Two re-created pairs that a review had removed from one side only: `as-prep`/`than-conj` dropped from both sides (comparison *as* is not the role preposition), `along-prep`/`up-prep` restored on `along-prep` sense 1 with a note. Lesson: a review that removes a cross-reference must remove its mirror too, or the next lint puts it back. The build cycles' own back-links (four cycles, 15 links) were checked as they were made: 4 misplaced.
