...
`--decide` now also takes `--index N` (1-based), applied after any `--quote` narrowing: it selects by position among the remaining candidates instead of by quote text, so a quote nested inside another quote on the same field/role no longer needs a manual `decisions.jsonl` line. The ambiguous-match listing now numbers its candidates (`[1]`, `[2]`, ...) so the session can read off which index to pass; an out-of-range index logs nothing and errors, same as an unmatched `--quote`. Tests in `tools/tests/test_review_panel.py` (`TestDecideSubstringCollision`, plus two new cases in `TestDecideCollisions`) reproduce this note's exact `pick-up-phrv` scenario (a full-sentence quote and its tail) and confirm `--quote` alone stays ambiguous while `--quote` plus `--index` resolves it.
