...
`--decide` now takes an optional `--quote "<exact or partial text>"`. When a field/role pair has more than one open issue, `--decide` without `--quote` prints every candidate (severity, family, quote, reason) and logs nothing, exit 1; `--quote` narrows to the matching issue(s), and still errors (logging nothing) if the text matches none or more than one. A single match needs no `--quote`, so every existing call for an unambiguous field/role still works unchanged. Tests in `tools/tests/test_review_panel.py` (`TestDecideCollisions`) cover the ambiguous-without-quote, disambiguated, no-match, and single-match cases. `they-pron`'s three already-logged `adaptation` decisions (all recorded `minor`, though two of the three source issues were `blocking`) predate the fix and are left as historical `reviews/decisions.jsonl` lines, not rewritten; the gate was never at risk (it compares totals, not per-verdict), only the precision breakdown for that one field.
